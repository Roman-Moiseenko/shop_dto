...
class DeactivateMenuItemUseCaseTest extends TestCase
{
    use MockPermission;

    private MenuItemRepositoryInterface $itemRepo;
    private DeactivateMenuItemUseCase $useCase;
    ...
    protected function setUp(): void
    {
        ...
        $this->useCase = new DeactivateMenuItemUseCase($this->itemRepo);
    }
    ...
    #[Test]
    public function deactivates_item_successfully(): void
    {
        $itemId = 42;
        $item = new MenuItemEntity(menuId: 1, title: 'Test');
        $item->id = $itemId;
        $item->isActive = true; // изначально активен
        ...
        $this->itemRepo->shouldReceive('save')
            ...
            ->with(Mockery::on(function (MenuItemEntity $savedItem) {
                return $savedItem->isActive === false;
            }))
            ->andReturn($item);
